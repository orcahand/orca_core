...
class OrcaHand:
    """
    OrcaHand class is used to control the hand of the robot with simple high level control methods in joint space.
   """
    def __init__(self, orca_config: str = "orca_config.yaml"):
        ...
        self.calib_path = os.path.join(os.path.dirname(__file__), "calibration", 'calibration.yaml')
        config = read_yaml(self.config_path)
        calib = read_yaml(self.calib_path)
            
        self.baudrate = config.get('baudrate', 3000000)
        self.port = config.get('port', '/dev/ttyUSB0')
        self.max_current = config.get('max_current', 300)
        self.control_mode: str = config.get('control_mode', 'current_position')
        
        self.calib_current = config.get('calib_current', 200)
        self.calib_step_size = config.get('calib_step_size', 0.1)
        self.calib_step_period = config.get('calib_step_period', 0.01)
        self.calib_threshold = config.get('calib_threshold', 0.01)
        self.calib_num_stable = config.get('calib_num_stable', 20)
        self.calib_sequence = config.get('calib_sequence', {})
    ...
    def set_max_current(self, current):
        ...
        Parameters:
        - current (float) or (list): If list, it should be the maximum current for each motor, otherwise it will be the same for all motors.
        """
    ...
    def set_control_mode(self, mode, motor_ids=None):
        """
    ...
    def _set_motor_pos(self, desired_pos, rel_to_current=False) -> None:
        """
    ...
    def init_joints(self, calibrate=False):
        """
        Initialize the joints and sets to the zero position.
        If the hand is not calibrated, it will assume the current position as the zero position and start the calib.
        
        Parameters:
        - calibrate (bool): If True, the hand will be calibrated
        
        In both cases, all joints will be set to the zero position
        """
    ...
    def calibrate(self):
        ...
        for step in self.calib_sequence:
            desired_increment, motor_reached_limit, directions, position_buffers, motor_reached_limit = {}, {}, {}, {}, {}
...
if __name__ == "__main__":
    ...
    time.sleep(1)
    hand.set_joint_pos({'index_abd': -0})
    time.sleep(1)
    # hand.set_joint_pos({'middle_abd': -40})
    # time.sleep(1)
    # hand.set_joint_pos({'pinky_abd': -40})
    # time.sleep(1)
    # hand.set_joint_pos({'thumb_abd': -40})
    # time.sleep(1)
