...
def build_parser() -> argparse.ArgumentParser:
    ...
    # The mode flag below decides whether the loop is engaged, so the shared
    # --no-engage-feedback would only contradict it.
...
class HandControlUI:
    ...
    def create_ui(self, root):
        ...
        ttk.Label(root, text="ORCA HAND CONTROL", style="Header.TLabel").pack(
...
class MotorSliderUI:
    ...
    def create_ui(self, root):
        ...
        ttk.Label(root, text="ORCA HAND MOTOR CONTROL", style="Header.TLabel").pack(
            anchor=tk.W, padx=12, pady=(12, 0)
        )
...
class JointFeedbackSliderUI:
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(self.root, text="JOINT-FEEDBACK CONTROL", style="Header.TLabel").pack(
            anchor=tk.W, padx=10, pady=(10, 0)
        )

        sliders_frame = ttk.LabelFrame(self.root, text="targets", padding=6)
        sliders_frame.pack(fill=tk.X, padx=8, pady=6)
        for joint in self.slider_joints:
            rom_min, rom_max = self.hand.config.joint_roms_dict[joint]
            row = ttk.Frame(sliders_frame, padding=4)
            row.pack(fill=tk.X)
            ttk.Label(row, text=joint, width=14).pack(side=tk.LEFT)
            ttk.Label(row, text=f"{rom_min:+4.0f}°", width=6, style="Dim.TLabel").pack(side=tk.LEFT)
            slider = ModernSlider(
                row,
            ...
            ttk.Label(row, text=f"{rom_max:+4.0f}°", width=6, style="Dim.TLabel").pack(side=tk.LEFT)
            target_label = ttk.Label(row, text="--", width=8, style="Ok.TLabel")
        ...
        readback.pack(fill=tk.X, padx=8, pady=6)
        for joint in self.slider_joints:
            row = ttk.Frame(readback)
            row.pack(fill=tk.X, pady=2)
            ttk.Label(row, text=joint, width=14).pack(side=tk.LEFT)
            self.measured_labels[joint] = ttk.Label(row, text="meas: --", width=14, style="Dim.TLabel")
            self.measured_labels[joint].pack(side=tk.LEFT)
            self.trim_labels[joint] = ttk.Label(row, text="", width=14, style="Dim.TLabel")
        ...
        tuning.pack(fill=tk.X, padx=8, pady=6)
        for label, var in (
            ("Kp", self.kp_var),
            ("Ki", self.ki_var),
    ...
    def _refresh(self) -> None:
        try:
            ...
            for joint in self.slider_joints:
                self.measured_labels[joint].config(text="meas: e-stop", style="Err.TLabel")
                self.trim_labels[joint].config(text="trim: --", style="Err.TLabel")
            logging.warning("joint loop unavailable: %s", exc)
            return
        for joint in self.slider_joints:
            self.measured_labels[joint].config(
                text=f"meas: {measured.get(joint, float('nan')):+6.1f}°", style="Dim.TLabel"
            )
            self.trim_labels[joint].config(
                text=f"trim: {correction.get(joint, 0.0):+5.1f}°", style="Dim.TLabel"
            )
