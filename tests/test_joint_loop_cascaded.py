"""Tests for ``JointLoopThreadCascaded``: bumpless start, vectorised
joint→motor mapping, asymmetric watchdog tiers (zero-trim at >200 ms,
e-stop without mode restore at >1000 ms), and the jitter monitor.
"""
...
def _make_loop(
    ...
    *,
    Kp=0.1,
    Ki=0.0,
    correction_max_rad=math.radians(15),
    target_hz=100,
):
    controller = CascadedJointController(
        num_joints=len(hand._encoder_backed_joints())
    )
    ...
    return JointLoopThreadCascaded(hand, encoder_source, controller, target_hz=target_hz)
...
def _expected_motor_pos(hand, joint: str, joint_command_rad: float) -> float:
    """Mirror ``OrcaHand._joint_to_motor_pos``: joint_roms is degrees,
    joint_to_motor_ratios is motor-rad per joint-deg, so the rad-valued
    cascaded target is converted to degrees before plugging in."""
    ...
    joint_command_deg = math.degrees(joint_command_rad)
...
def test_bumpless_first_step_writes_base_motor_target_with_no_correction(calibrated_hand):
    ...
    base motor target for the measured pose. The latched target is the
    decoded encoder reading (subject to ~1e-4 rad quantisation)."""
    measured_angle = 0.05
    encoder = StaticEncoderSource(
        encoder_reading_from_joint_angles(
            {j: measured_angle for j in calibrated_hand._encoder_backed_joints()}
        )
    )
    loop = _make_loop(calibrated_hand, encoder, Kp=1.0)
    loop.prime_for_step()
    primed_targets = {
        joint: loop._target_rad[loop._joint_names.index(joint)]
...
def test_step_once_applies_correction_to_motor_target(calibrated_hand):
    """The full data path: encoder → controller → vectorised joint→motor →
    ...
    Kp, target_offset = 0.5, 0.1
    loop = _make_loop(calibrated_hand, encoder, Kp=Kp)
    loop.prime_for_step()
    loop.set_target(
        {j: target_offset for j in calibrated_hand._encoder_backed_joints()}
    )
    loop.step_once(dt=0.01)

    expected_correction = Kp * target_offset
    correction = loop.get_correction()
    for joint in calibrated_hand._encoder_backed_joints():
        assert correction[joint] == pytest.approx(expected_correction, abs=1e-9)
        motor_id = calibrated_hand.config.joint_to_motor_map[joint]
        expected_motor = _expected_motor_pos(
            calibrated_hand, joint, target_offset + expected_correction
        )
...
def test_does_not_snapshot_or_swap_operating_modes(calibrated_hand):
    """Cascaded mode keeps the bus in current_based_position throughout.
    ``prime_for_step`` must not capture or modify operating modes."""
    pre_modes = calibrated_hand._motor_client.get_operating_modes()
    encoder = _static_at_zero(calibrated_hand)
    loop = _make_loop(calibrated_hand, encoder)
    loop.prime_for_step()

    assert calibrated_hand._motor_client.get_operating_modes() == pre_modes
...
def test_tier1_warns_with_rate_limit_but_continues_normal_control(calibrated_hand, caplog):
    ...
    assert np.all(loop._controller.get_state()["ierr_rad"] > 0.0)
    freshness_records = [
...
def test_tier2_freezes_integrator_and_recovers(calibrated_hand):
    ...
    ierr_before = loop._controller.get_state()["ierr_rad"].copy()

    encoder.freshness_ms = WATCHDOG_HOLD_MS + 10
    for _ in range(10):
        loop.step_once(dt=0.005)
    np.testing.assert_allclose(loop._controller.get_state()["ierr_rad"], ierr_before)
...
def test_tier3_zeroes_trim_writes_base_motor_target_only(calibrated_hand):
    ...
    target for the (uncorrected) joint target. The inner motor PID holds
    the un-trimmed pose."""
    ...
    loop.prime_for_step()
    target = 0.07
    loop.set_target({j: target for j in calibrated_hand._encoder_backed_joints()})
    loop.step_once(dt=0.005)

    for joint in calibrated_hand._encoder_backed_joints():
        motor_id = calibrated_hand.config.joint_to_motor_map[joint]
        expected = _expected_motor_pos(calibrated_hand, joint, target)
        assert calibrated_hand._motor_client._pos[motor_id] == pytest.approx(
            expected, abs=1e-6
        )
...
def test_tier4_sets_fallback_without_restoring_modes(calibrated_hand):
    """Tier-4 stops the loop without touching operating modes — none were
    swapped on connect, so there's nothing to restore. The bus stays in
    current_based_position holding the last commanded position."""
    pre_modes = calibrated_hand._motor_client.get_operating_modes()
    encoder = _static_at_zero(calibrated_hand)
    loop = _make_loop(calibrated_hand, encoder)
    loop.prime_for_step()
...
def test_cascaded_motor_pos_matches_open_loop_kinematics(calibrated_hand):
    """The cascaded loop's vectorised joint→motor mapping must agree with
    ``OrcaHand._joint_to_motor_pos`` for the same joint target. This is
    the regression for the units bug where joint_command (radians) was
    plugged into a formula that expects degrees, masking a 57× scale
    mismatch on every motor write.
    """
    encoder = _static_at_zero(calibrated_hand)
    loop = _make_loop(calibrated_hand, encoder)
    loop.prime_for_step()
    ...
    target_rad = {
        joint: math.radians(deg)
        for joint, deg in zip(encoder_joints, [0.1, -0.1, 0.5, -0.5][: len(encoder_joints)])
    }
    target_array = np.array(
        [target_rad[j] for j in loop._joint_names], dtype=np.float64
    )
    cascaded_motor = loop._joint_to_motor_pos(target_array)

    open_loop_motor = calibrated_hand._joint_to_motor_pos(
        {joint: math.degrees(rad) for joint, rad in target_rad.items()}
    )
    ...
    for idx, joint in enumerate(loop._joint_names):
        ...
        assert cascaded_motor[idx] == pytest.approx(expected, abs=1e-9)
