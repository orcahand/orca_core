...
def few_anchor_samples(monkeypatch):
    """Average a handful of frames instead of the production 200.

    The mock stream repeats one count, so the anchor is identical either way;
    driving 200 frames through the link just costs a second per test.
    """
    import orca_core.hardware.joint_encoder_client as module

    real = module.sample_anchor_count_from_client
    monkeypatch.setattr(
        module,
        "sample_anchor_count_from_client",
        lambda client, slot, num_samples=8, **kw: real(
            client, slot, num_samples=num_samples, **kw
        ),
    )
