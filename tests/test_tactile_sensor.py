...
def test_capture_counts_frames_that_share_a_coarse_timestamp(tactile_mock_factory, monkeypatch):
    ...
    from orca_core.hardware.sensing.types import TactileReading, TaxelReading
    ...
    monkeypatch.setattr(
        client,
        "get_latest",
        lambda: TactileReading(
            forces=None, taxels=TaxelReading(taxels=next(payloads), timestamp=0.0)
        ),
    )
