...
from .waypoint_fit import run_waypoint_fit_for_step
...
def _drive_calibration(
    ...
    *,
    ...
    for step_index, step in enumerate(calibration_sequence):
        ...
        # so the yaml is written once, with corrected values.
        # A joint whose anchor sample failed this run keeps a stale anchor on a
        # slot that just misbehaved; fitting against it would fold that error in.
        fit_joints = [j for j in completed_this_step if j not in joints_to_anchor]
        if (
            encoder_pass_active
