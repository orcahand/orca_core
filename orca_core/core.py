...
class OrcaHand:
    ...
    def calibrate(self, blocking: bool = True, joints: list = None, force_wrist: bool = False):
        if blocking:
            self._task_stop_event.clear()
            self._calibrate(joints=joints, force_wrist=force_wrist)
        else:
            self._start_task(self._calibrate, joints=joints, force_wrist=force_wrist)

    def _calibrate(self, joints: list = None, force_wrist: bool = False):
        ...
        if joints is not None:
            ...
            for step in calib_sequence:
                ...
                if filtered_joints:
                    filtered_sequence.append({"step": step["step"], "joints": filtered_joints})
