...
comes from the motor bus itself. Hands that report no side are treated as
right-handed; a config selects the model explicitly whenever detection can't.
"""
...
def detect_hand() -> HandDetection:
    ...
    hand whose motor side is a bare USB adapter is found by the adapter's
    vendor ID and confirmed by that same probe. Anything the hardware doesn't
    answer falls back
