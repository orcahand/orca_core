"""Cascaded outer-loop trim thread.

Each cycle reads joint angles from a ``JointEncoderClient``, asks the
``CascadedJointController`` for a per-joint correction (radians), maps
``target + correction`` to motor positions, and writes via
``write_desired_pos``. The inner Dynamixel position PID tracks the motor
target against the motor encoder; this thread trims the residual offset
between motor angle and joint angle.

Encoder-freshness watchdog (the inner motor PID continues to hold the
last commanded position even without host updates, so the higher tiers
do not drop torque):
...
class JointLoopThreadCascaded:
    ...
    def __init__(
        ...
        target_hz: int = DEFAULT_CASCADED_LOOP_HZ,
    ):
    ...
    def set_target(self, joint_targets: JointTargets) -> None:
        """Update the joint setpoint. Accepts a ``{joint_name: angle_rad}``
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(
            target=self._run_loop, name="JointLoopThreadCascaded", daemon=True
        )
        self._thread.start()
    ...
    def _joint_to_motor_pos(self, joint_command_rad: np.ndarray) -> np.ndarray:
        """Vectorised joint→motor mapping for the snapshotted joint set.

        ``joint_roms_dict`` is degrees and ``joint_to_motor_ratios_dict`` is
        motor-radians per joint-degree, so the input must be converted to
        degrees before applying the formula. See
        :meth:`OrcaHand._joint_to_motor_pos` for the equivalent scalar
        implementation.
        """
        joint_command_deg = np.degrees(joint_command_rad)
        inverted_term = (
    ...
    def _trigger_estop(self, reason: str) -> None:
        """Set ``fallback_active`` and signal the thread to stop. The inner
        motor PID holds the last commanded position; no operating-mode
        restore is needed because none were swapped on connect."""
        if self._stats["fallback_active"]:
            return
        logger.error("cascaded joint loop e-stop: %s", reason)
