...
from orca_core.hardware.sensing.taxel_coordinates import get_all_coordinates, MODELS_DIR, _load_finger_mapping
...
@app.route('/api/sensor_model_info')
def sensor_model_info():
    """Return finger-to-model mapping with STL file availability."""
    mapping = _load_finger_mapping()
    result = {}
    for finger, model_name in mapping.items():
        stl_path = os.path.join(MODELS_DIR, model_name, f'{model_name}.stl')
        result[finger] = {
            'model': model_name,
...
@app.route('/api/stl_file/<model_name>')
def stl_file(model_name):
    """Serve the STL file for a sensor model."""
    stl_path = os.path.join(MODELS_DIR, model_name, f'{model_name}.stl')
